...
- **Phase 4: Personalized Home Feed Tab**: Added Home as the first/default tab. New files: `client/screens/HomeScreen.tsx`, `client/navigation/HomeStackNavigator.tsx`. API: `GET /api/feed` returns user's vehicles, recent bay threads (from joined garages), solved threads matching vehicle makes ("For Your Garage"), and recent Swap Shop listings. Home screen sections: greeting, Your Vehicles (horizontal cards), Recent Activity (bay threads), For Your Garage (solved threads by make), New in Swap Shop (listings). Empty state prompts to add vehicle/join bays. Tab order: Home, Bays, Swap Shop, Notes, TorqueAssist, Tool & Gear. DesktopSidebar updated with Home nav item.
